...
public class DiezmoDAO {
    ...
    public DiezmoDAO(VistaDiezmo vd){
        this.vistaDiezmo=vd;
    }
    ...
    public boolean agregar(Diezmo di){
        int res=0;
        String agregarsql="insert into diezmo(idlider,carnet,mes,entrada,salida,saldoanterior,saldoactual,descripcion,fecharegistro)"
                +"values(?,?,?,?,?,?,?,?,?)";
    ...
    public boolean modificar(Diezmo dm){
        int res=0;
        String modificarsql="update diezmo set idlider=?,carnet=?,mes=?,entrada=?,salida=?,saldoanterior=?,saldoactual=?,descripcion=?,fecharegistro=?"
                            +"where iddiezmo=?";
        try {
            con=conectarMySQL.conectando();
